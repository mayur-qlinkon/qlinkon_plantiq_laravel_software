        ...
        <form action="{{ route('admin.purchase-returns.store') }}" method="POST"
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
                <div class="p-6 grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Status <span class="text-red-500">*</span></label>
                        <x-custom-select
                            name="status"
                            :options="[
                                'draft' => 'Draft',
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Return From Warehouse <span class="text-red-500">*</span></label>
                        ...
                        <div @change="header.warehouse_id = $event.target.value">
                            <x-custom-select name="warehouse_id" :options="$warehouseOptions"
                                ...
                                @set-return-warehouse.window="value = $event.detail" />
                        </div>
                        <p class="text-[11px] text-gray-400 mt-1">Defaults to the purchase warehouse. Change it if the stock was transferred to another warehouse.</p>
                    </div>
                </div>
            </div>
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors flex flex-col md:table-row border-b border-gray-200 md:border-none p-4 md:p-0 relative"
                                    ...
                                    <td class="px-4 md:py-3 py-2 flex items-center justify-between md:table-cell">
                                        ...
                                        <div class="flex items-center gap-1.5 md:w-[130px] justify-center"
                                            ...
                                            <input type="number" step="0.01" min="0" :max="item.max_qty"
                                                x-model="item.quantity"
                                                @keydown="window.preventInvalidChars($event)"
                                                @input="if(item.quantity < 0 || item.quantity === '') item.quantity = 0; if(item.quantity > item.max_qty) item.quantity = item.max_qty; calculate();"
                                    ...
                                    <td class="px-4 md:py-3 py-2 flex flex-col md:table-cell gap-1.5 md:gap-0">
                                        ...
                                        <div class="w-full"
                                            ...
                                            <button type="button" role="combobox" x-ref="trigger"
                                                :aria-expanded="open" :disabled="!item.is_included"
                                                @click="toggle()"
                                                :class="open ? 'border-brand-500 ring-2 ring-brand-500/15' : 'border-gray-300 hover:border-gray-400'"
                                                ...
                                                <span class="flex-1 truncate">
                                                    @foreach ($returnReasonOptions as $value => $label)
                                                        <span x-show="item.return_reason === '{{ $value }}'">{{ $label }}</span>
                                                    @endforeach
                                                </span>
                                            ...
                                            <div x-cloak x-show="open" @click.away="open = false" role="listbox"
                                                :style="`top:${top}px; left:${left}px; width:${width}px;` + (above ? ' transform: translateY(-100%);' : '')"
                                                ...
                                                @foreach ($returnReasonOptions as $value => $label)
                                                    <button type="button" role="option"
                                                        ...
                                                        :class="item.return_reason === '{{ $value }}' ? 'bg-brand-500/10 text-brand-600 font-bold' : 'text-gray-700 hover:bg-gray-50'"
                                                        ...
                                                        <svg class="h-3.5 w-3.5 shrink-0"
                                                            x-show="item.return_reason === '{{ $value }}'" fill="none"
        ...
        function purchaseReturnForm(initialPurchase = null, allUnits = []) {
            return {
                ...
                async searchPOs() {
                    ...
                    try {
                        let response = await fetch(`{{ route('admin.api.purchases.search') }}?term=${encodeURIComponent(term)}`);
                ...
                loadPurchaseData(data) {
                    ...
                    // header.warehouse_id, so the PO's warehouse is pushed in.
                    this.$nextTick(() => {
