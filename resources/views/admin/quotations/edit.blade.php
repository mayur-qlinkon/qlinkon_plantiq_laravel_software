...
@section('content')
    @php
        // Prepare options arrays with string keys for custom select matching
        $warehouseOptions = [];
        ...
        // Same 'fixed'/'percentage' values the validator expects — matches
        // what the create blade uses. Falls back to the quotation's own saved
// value (not just old()) so the trigger's label is correct on the
        // very first load, before any validation-failure re-render exists.
        ...
        @if ($quotation->status === 'draft')
            <div class="bg-amber-50 border border-amber-200 text-amber-800 rounded-lg px-4 py-3 mb-6 flex items-start gap-3">
                ...
                <div class="text-sm">
                    ...
                    <div class="text-xs">Keep editing, or use <strong>Save &amp; Mark as Sent</strong> to record that it was
                        shared with the customer.</div>
                </div>
            </div>
        @endif
        ...
        @if ($errors->any())
            <div class="bg-red-50 text-red-700 p-4 rounded-lg mb-6 border border-red-200 shadow-sm">
                <div class="font-bold mb-2 flex items-center gap-2">
                    <i data-lucide="alert-triangle" class="w-5 h-5"></i> Please fix the following mistakes:
                </div>
                    ...
                    <div>
                        ...
                        <div @change="formData.gst_treatment = $event.target.value; calculate();">
                            <x-custom-select name="gst_treatment" :options="[
                                ...
                                'sez' => 'SEZ',
                            ]" :selected="old('gst_treatment', $quotation->gst_treatment)"
                                @set-gst-treatment.window="value = $event.detail" />
                        </div>
                    </div>

                    <input type="hidden" name="store_id" value="{{ $quotation->store_id }}">
                    ...
                    <div>
                        ...
                        <div @change="formData.warehouse_id = $event.target.value">
                            <x-custom-select name="warehouse_id" placeholder="Select Warehouse" :options="$warehouseOptions"
                                :selected="old(
                                    'warehouse_id',
                                    $quotation->warehouse_id ? (string) $quotation->warehouse_id : '',
                    ...
                    <div class="mt-[-1px] space-y-1.5">
                        ...
                        <div @change="formData.supply_state = $event.target.value; calculate();">
                            <x-custom-select name="supply_state" id="supply_state" placeholder="Select State"
                                class="font-bold text-[#108c2a]" :options="$stateOptions" :selected="old('supply_state', $quotation->supply_state)"
                            ...
                            <template x-for="(item, index) in items" :key="item.key">
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-center align-middle">
                                        {{-- A missing code needs to be obvious before saving,
                                             not at GST filing time. --}}
                                        <span x-show="item.hsn_code" class="text-[12px] font-mono text-gray-600"
                                            x-text="item.hsn_code"></span>
                                        <span x-show="!item.hsn_code"
                                            class="inline-block rounded border border-red-200 bg-red-50 px-1.5 py-0.5 text-[10px] font-bold text-red-600">
                                            Missing
                                        </span>
                                    </td>
                    ...
                    <div class="space-y-3 text-sm text-gray-600">
                        ...
                        <div class="flex flex-wrap sm:flex-nowrap justify-between items-center pt-2 gap-2">
                            <div class="flex items-center gap-2">
                                ...
                                <div class="w-36" @change="global.discount_type = $event.target.value; calculate();">
                                    <x-custom-select name="discount_type" :options="$discountTypeOptions" :selected="old('discount_type', $quotation->discount_type ?? 'fixed')" />
                                </div>
                            </div>
                            <input type="text" inputmode="numeric" pattern="[0-9]*" name="discount_value"
                                x-model="global.discount_value" @keydown="window.preventInvalidChars($event)"
                        ...
                        <div
                            ...
                            <input type="text" inputmode="numeric" pattern="[0-9]*" name="shipping_charge"
                                x-model="global.shipping" @keydown="window.preventInvalidChars($event)"
                ...
                <div class="p-6 space-y-5 flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    <div>
                        ...
                        <div @change="activeEditData.tax_type = $event.target.value">
                            <x-custom-select name="modal_tax_type" :options="$itemTaxTypeOptions"
                                selected="exclusive" placeholder="Select tax type"
                    ...
                    <div>
                        ...
                        <div @change="activeEditData.discount_type = $event.target.value">
                            <x-custom-select name="modal_discount_type" :options="$itemDiscountTypeOptions"
                                selected="percentage" placeholder="Select type"
                    ...
                    <div>
                        ...
                        <x-alpine-select model="activeEditData.unit_id" items="units" item-key="id"
                            item-label="item.name" placeholder="Select unit"
        ...
        function quotationForm(allUnits = [], companyState = '', allClients = [], existingData = null) {
            ...
            return {
                ...
                formData: {
                    ...
                    store_id: existingData?.store_id || "{{ $quotation->store_id ?? '' }}",
                    warehouse_id: existingData?.warehouse_id ? String(existingData.warehouse_id) :
                        "{{ $warehouses->firstWhere('is_default', true)?->id ?? ($warehouses->first()?->id ?? '') }}",
                },

                // 🌟 Pre-fill Global Financials
                global: {
                    shipping: parseInt(existingData?.shipping_charge || 0, 10),
                    discount_type: existingData?.discount_type || 'fixed',
                    discount_value: parseInt(existingData?.discount_value || 0, 10),
                ...
                init() {
                    ...
                    // 🌟 Boot complete hote hi custom select dropdown ko state sync karne ke liye force karenge
                    this.$nextTick(() => {
