...
@section('header-title')
    <h1 class="text-sm font-bold tracking-widest text-gray-500 uppercase">Edit / Roles / {{ $role->name }}</h1>
@endsection
...
@section('content')
    @php
        ...
        foreach ($permissions as $moduleGroup => $features) {
            ...
            foreach ($features as $featureName => $perms) {
                // Wrapped in collect() to prevent the array pluck error!
    ...
    {{-- Note: We pass $rolePermissions as the fallback for old() to pre-select existing permissions --}}
    <div class="pb-10" x-data="rolePermissionsForm(@js(old('permissions', $rolePermissions)), @js($jsMatrix))">
        {{-- HEADER --}}
        <div class="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-[#212538]">Edit Role: {{ $role->name }}</h2>
                <p class="mt-1 text-[13px] font-medium text-gray-500">Modify access permissions based on active module
                    licenses.</p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <button type="submit" form="role-form"
                    ...
                    <i data-lucide="save" class="h-4 w-4"></i> Update Role
                </button>
            </div>
        </div>
