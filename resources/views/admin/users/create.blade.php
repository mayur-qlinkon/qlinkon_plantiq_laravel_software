...
@section('header-title')
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">Add New Staff Member</h1>
            <p class="mt-1 text-sm text-gray-500">Create a new account and configure their access permissions.</p>
        </div>
    </div>
@endsection
...
@section('content')
    ...
    <div class="w-full pb-10" x-data="{
        ...
        statusActive: {{ old('status', 'active') == 'active' ? 'true' : 'false' }},
        showPassword: false,
        avatarPreview: null,
        handleImageUpload(e) {
        ...
        {{-- Main Form Container --}}
        <form action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data" class="w-full">
            @csrf
                ...
                <div class="space-y-6 lg:col-span-5">
                    ...
                    <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
                        ...
                        <div class="space-y-4">
                            ...
                            {{-- Phone Number --}}
                            <div>
                                <label class="mb-2 block text-[13px] font-bold text-gray-700">Phone Number <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div class="relative">
                                <label class="mb-2 block text-[13px] font-bold text-gray-700">Password <span
                                        class="text-red-500">*</span></label>
                                <div class="relative">
                                    ...
                                    <button @click="showPassword = !showPassword" type="button"
                                        ...
                                        <i data-lucide="eye-off" class="h-4 w-4" x-show="showPassword"></i>
                                    </button>
                                </div>
                            </div>
                        </div>
                ...
                <div class="space-y-6 lg:col-span-7">
                    ...
                    <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
                        ...
                        @php
                            ...
                            $preSelectedStores = collect(old('store_ids', []))
                                ->map(fn($id) => (string) $id)
                                ->values()
                                ->all();
                        @endphp
                    ...
                    @if ($assignableModules->isNotEmpty())
                        <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
                            ...
                            <p class="mb-4 text-xs text-gray-500">Only modules included in your current plan are shown.
                                This list automatically scales for your license count.</p>
                                ...
                                <div x-ref="list"
                                    ...
                                    @foreach ($assignableModules as $module)
                                        @php
                                            ...
                                            $isChecked = in_array($module->id, old('module_ids', []));
                                            $seatLimit = $license?->seat_limit;
                                            $seatUsed = $seatsUsed[$module->id] ?? 0;
                                            $seatFull = $license && !is_null($seatLimit) && $seatUsed >= $seatLimit;
                                        @endphp
                                        <label data-module-name="{{ Str::lower($module->name) }}"
                                            ...
                                            @if ($license && !is_null($seatLimit))
                                                <span
                                                    ...
                                                    {{ $seatFull ? 'Limit Reached' : "$seatUsed/$seatLimit" }}
                                                </span>
                                            @endif
                                        </label>
                                    @endforeach
                    ...
                    <div class="mt-8 flex flex-col-reverse justify-end gap-3 border-t border-gray-200 pt-6 sm:flex-row">
                        <a href="{{ route('admin.users.index') }}"
                            class="rounded-xl border border-gray-300 bg-white px-8 py-3 text-sm font-medium text-gray-700 shadow-sm transition-colors hover:bg-gray-50">
                            Cancel
                        </a>
                        <button type="submit"
                            ...
                            <i data-lucide="user-plus" class="h-4 w-4"></i> Create Staff Member
                        </button>
                    </div>
                </div>
            </div>
