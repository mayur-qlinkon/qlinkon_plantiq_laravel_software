...
@section ('heading', 'Welcome to Plantiq')
@section ('subheading', 'Sign in to your workspace to continue')

@section ('content')
    <form method="POST" action="{{ tenant_url('admin') }}" novalidate class="space-y-5" x-data="{ showPwd: false }">
        @csrf
        ...
        {{-- Accounts are provisioned by the Qlinkon team --}}
    ...
    {{-- ═══ PWA Dynamic Sleek Floating Banners ═══ --}}
    <div x-data="pwaInstall()" x-cloak>
        {{-- Android / Chrome / Edge — Slim & Non-Intrusive Toast --}}
        <div
        ...
        {{-- Already installed — One-time Auto Dismissible Popup --}}
        <div
...
@push ('pwa')
    <link rel="manifest" href="{{ tenant_url('manifest.json') }}" />
    <meta name="theme-color" content="#82cd47" />
    <link class="apple-touch-icon" href="{{ asset('assets/icons/favicon.png') }}" />
@endpush
                ...
                init() {
                    ...
                    if ("serviceWorker" in navigator) {
                        navigator.serviceWorker
                            .register("/sw.js", {
                                scope: "{{ tenant_mode() === 'slug' && tenant() ? '/' . tenant()->slug . '/' : '/' }}",
                            })
                            .catch(() => {});
                    }
                },
