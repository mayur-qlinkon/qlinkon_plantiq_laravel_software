...
@section('title', 'Edit Profile Kit')
@section('header', 'Edit Profile Kit')
    ...
    <div class="mx-auto">
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                ...
                <h1 class="text-2xl font-extrabold text-gray-900 tracking-tight">Edit Kit: {{ $profileKit->name }}</h1>
                <p class="text-sm text-gray-500 mt-1">Modify physical tag details or update pricing.</p>
            </div>
            <button type="submit" form="edit-kit-form" class="bg-brand-600 hover:bg-brand-700 text-white px-6 py-2.5 rounded-lg text-sm font-semibold transition-all shadow-md shadow-brand-500/20 flex items-center justify-center gap-2 self-start sm:self-end">
        ...
        @if ($errors->any())
            <div class="mb-6 flex items-start gap-3 bg-red-50 border border-red-200 text-red-800 text-sm px-5 py-4 rounded-xl shadow-sm">
                ...
                <div>
                    <span class="font-bold">Could not update the kit. Please check the errors below:</span>
        ...
        <form id="edit-kit-form" action="{{ route('platform.profile-kits.update', $profileKit->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name', $profileKit->name) }}" required
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="4"
                                    class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all resize-none @error('description') border-red-300 ring-red-100 @enderror">{{ old('description', $profileKit->description) }}</textarea>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-2">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" step="0.01" name="price" value="{{ old('price', $profileKit->price) }}" required min="0" 
                                ...
                                <div>
                                    ...
                                    <input type="number" name="sort_order" value="{{ old('sort_order', $profileKit->sort_order) }}"
                ...
                <div class="space-y-8">
                    ...
                    {{-- Image Upload Card --}}
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8" x-data="{ preview: '{{ $profileKit->image ? asset('storage/' . $profileKit->image) : '' }}', isDragging: false }">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
                        ...
                        <label class="flex items-center justify-between p-4 border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 cursor-pointer transition-all">
                            ...
                            <div class="relative inline-flex items-center cursor-pointer ml-4 shrink-0">
                                ...
                                <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $profileKit->is_active) ? 'checked' : '' }}>
