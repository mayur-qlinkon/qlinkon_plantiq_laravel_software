...
<body class="overflow-x-hidden text-gray-900 antialiased" x-data="{
    ...
    <nav
        ...
        <div class="relative z-20 ml-auto flex items-center gap-2 sm:gap-3">
            @if ($isLoggedIn && $hasActiveSubscription)
                {{-- Subscribed user → straight to dashboard --}}
                <a href="{{ $dashboardUrl }}"
    ...
    {{-- <section
        class="reveal mx-auto max-w-7xl px-4 py-8 text-center sm:px-6 lg:px-8"
        x-show="pageData && pageData.trust_companies"
    >
        <p
            class="mb-10 text-lg font-medium text-gray-500 md:text-xl"
            x-text="pageData?.trust_heading || 'Trusted by Nursery Owners'"
        ></p>

        <div class="marquee-wrapper relative flex w-full overflow-hidden">
            <div class="animate-marquee min-w-full shrink-0 items-center justify-start gap-8 pr-8 md:gap-16 md:pr-16">
                <template x-for="[key, company] in Object.entries(pageData?.trust_companies || {})" :key="key">
                    <div
                        class="flex shrink-0 cursor-pointer items-center justify-center transition-transform duration-300 hover:scale-105"
                    >
                        <template x-if="company.logo">
                            <img
                                :src="`https://qlinkon.com/storage/${company.logo}`"
                                :alt="company.name"
                                class="h-16 w-auto max-w-[200px] object-contain md:h-20"
                                loading="lazy"
                                decoding="async"
                            />
                        </template>
            ...
            <div
                class="animate-marquee min-w-full shrink-0 items-center justify-start gap-8 pr-8 md:gap-16 md:pr-16"
                aria-hidden="true"
            >
                <template x-for="[key, company] in Object.entries(pageData?.trust_companies || {})" :key="key + '-dup'">
                    <div
                        class="flex shrink-0 cursor-pointer items-center justify-center transition-transform duration-300 hover:scale-105"
                    >
                        <template x-if="company.logo">
        ...
        <div class="mb-16 grid grid-cols-1 items-start gap-10 sm:grid-cols-2 md:grid-cols-5 md:gap-8">
            ...
            <div>
                ...
                <ul class="space-y-4 text-sm font-medium text-gray-500">
                    <li><a href="{{ url('/about') }}" class="transition-colors hover:text-gray-900">About Us</a></li>
                    <li>
                    ...
                    <li><a href="{{ url('/contact') }}" class="transition-colors hover:text-gray-900">Contact</a>
                    </li>
                    <li>
